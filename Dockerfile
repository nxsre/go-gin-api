...
# COPY 源路径 目标路径
COPY . $GOPATH/src/github.com/nxsre/go-gin-api

# RUN 执行 go build .
RUN cd $GOPATH/src/github.com/nxsre/go-gin-api && go build .
...
COPY --from=builder /opt/repo/src/github.com/nxsre/go-gin-api /opt
